    ...
    fun getPlanDayNameByDayId(strId: String): String {
        ...
        try {
            ...
            if (cursor != null && cursor.count > 0) {
                ...
                if(weekName.toInt()>1)
                {
                    dayName = (cursor.getString(cursor.getColumnIndexOrThrow(DayName)).toInt()+((weekName.toInt()-1)*7)).toString()
                }else {
        ...
        try {
            ...
            if (cursor != null && cursor.count > 0) {
                while (cursor.moveToNext()) {
                    ...
                    aClass.isPro =  cursor.getString(cursor.getColumnIndexOrThrow(mIsPro)).equals("true")
                    aClass.isFromRaw =  cursor.getString(cursor.getColumnIndexOrThrow(mIsFromRaw)).equals("true")
                    arrPlan.add(aClass)
                }
            }
        } catch (e: Exception) {
